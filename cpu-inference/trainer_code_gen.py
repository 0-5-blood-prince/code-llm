...
def main():
    ...
    #     model = ipex.optimize(model, dtype=torch.float32, level="O1", inplace=True)
    ...
    if args.jit:
        ...
        for key in sig.parameters:
            if key in jit_inputs:
                jit_dict[key] = (jit_inputs[key])
        # jit_inputs = list(jit_inputs[key] for key in sig.parameters if jit_inputs[key] is not None)
        # print(jit_inputs.keys())
        traced_model = torch.jit.trace(model, example_kwarg_inputs=jit_dict, strict=False)
        traced_model = torch.jit.freeze(traced_model.eval())
        traced_model(**jit_dict)
        traced_model(**jit_dict)
